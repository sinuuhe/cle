...
Class PagoInscripcion {
    ...
    public function listar_pago($id) {
        $sql = "SELECT a.id as matricula, CONCAT(a.nombre, ' ', a.apellidoP, ' ', a.apellidoM) AS nombre, pi.monto_pago, pi.status,pi.fecha_pago, pi.id, pi.total_pago, pi.FORMA_PAGO
            FROM alumnos a 
            INNER JOIN pago_inscripciones pi on a.id=pi.id_alumno WHERE pi.id = '$id'";
