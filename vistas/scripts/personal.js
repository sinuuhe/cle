...
function baja(maestroId, nombre){
	alertify.dialog('confirm').
            set({
                transition: 'slide',
                message: 'Desea dar de baja al empleado ' + nombre + '?',
                'reverseButtons': true,
                'labels': {ok: 'Confirmar', cancel: 'Cancelar!'},
                onok: function () {
						$.post("../ajax/personal.php?op=baja", {id : maestroId}, function(e){
							alertify.success(e);
							tabla.ajax.reload();
						});	
                },
...
function alta(maestroId, nombre){
	...
	set({
		transition: 'slide',
		message: "¿Seguro que desea dar de alta al empleado " + nombre + "?",
		'reverseButtons': true,
		'labels': {ok: 'Confirmar', cancel: 'Cancelar!'},
		onok: function () {
				$.post("../ajax/personal.php?op=alta", {id : maestroId}, function(e){
					alertify.success(e);
					tabla.ajax.reload();
				});	
		},
...
function guardaryeditar(e)
{
	...
	$.ajax({
	    ...
	    {                    
	          alertify.alert("Registro de empleados",datos, function(){
				mostrarform(false);
	          	tabla.ajax.reload();
			  });	          
	    }
...
{
	$.post("../ajax/personal.php?op=mostrar",{id : idAlumno}, function(data, status)
	{
